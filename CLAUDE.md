...
- **Nothing in the process may write to a mail account.** mbsync is configured
  `Sync Pull` / `Create Near` / `Remove None`, with `Expunge None` by default
  and `Expunge Near` — which deletes local files only — for an account that
  opts into `expunge_local`. The file is generated by the program rather than
  mounted, so the directives cannot be edited into something that pushes.
  There is no send, delete, move or tag path against any account. The one
  local deletion in Go code is the account's own `-recent` bootstrap store,
  removed once the full mirror covers it.
